...
OUTLINE_KIND_LABEL = {"category": "总纲", "volume": "卷", "arc": "近纲", "chapter": "章",
                      "scene": "场景", "topic": "子题"}
...
def build_assembly(project_id: str, node_id: str, *, log: bool = True,
    ...
    body_inject = bool(get_settings()["assembly"].get("body_inject"))
    ...
    with tx() as conn:
        ...
        # WPS 式大纲(任务词 2026-09-09,默认关;方案书 W2"邻近原则"):本章祖先链、
        # 同级 topic(与章同父)与章直属子 topic 的 body 摘要(各截 300 字)按
        # on_demand 进装配——超限先裁,老书 body 全空=零变化
        if body_inject:
            for n in chain:
                if n["id"] == node_id:
                    continue
                ...
                if b:
                    sections.append({
                        "source": "outline:body", "kind": "on_demand",
                        "title": (f"{OUTLINE_KIND_LABEL.get(n['kind'], n['kind'])}"
                                  f"·{n['title']}(正文摘要)"),
                        "content": b[:300] + ("…(截断)" if len(b) > 300 else ""),
                        "included": True,
                    })
            near_topics = conn.execute(
                "SELECT title, body FROM outline_nodes"
                " WHERE kind='topic' AND (parent_id IS ? OR parent_id = ?)"
                " ORDER BY sort_order",   # 邻近 topic:与章同级 + 章直属子题
                (node["parent_id"], node_id)).fetchall()
            for t in near_topics:
                b = (t["body"] or "").strip()
                if b:
                    sections.append({
                        "source": "outline:body", "kind": "on_demand",
                        "title": f"子题·{t['title']}(正文摘要)",
                        "content": b[:300] + ("…(截断)" if len(b) > 300 else ""),
                        "included": True,
                    })
        ...
        # 二期③消费者 a:本章人物(cast)按需注入——人物板"同框"边 at_chapter=本章
        # 章号,端点人物 ref 的 L1 条目精准注入(即便 presence=按需未命中计划卡)。
        m_no = re.search(r"第(\d+)章", node["title"] or "")
        if m_no:
            no = int(m_no.group(1))
            cast = [dict(r) for r in conn.execute(
                "SELECT DISTINCT e.id AS entry_id, e.name AS name, e.content AS content"
                " FROM graph_edges ge"
                " JOIN graph_boards b ON b.id = ge.board_id"
                " JOIN graph_nodes n ON n.id IN (ge.from_node_id, ge.to_node_id)"
                " JOIN l1_entries e ON e.id = n.ref_id"
                " WHERE b.project_id=? AND b.kind='character'"
                "   AND ge.kind='同框' AND json_extract(ge.style,'$.at_chapter')=?"
                "   AND n.ref_type='l1_entry' AND n.ref_id IS NOT NULL"
                "   AND e.project_id=? AND e.entry_status='confirmed'",
                (project_id, no, project_id))]
            have_ids = {s.get("entry_id") for s in sections
                        if s.get("entry_id") and s.get("included")}
            for e in cast:
                if e["entry_id"] in have_ids:
                    continue   # 已随常驻装入装配,不重复注入
                sections.append({
                    "source": "l1:cast", "kind": "always",
                    "title": f"本章人物·{e['name']}",
                    "content": (e["content"] or "").strip() or "(档案内容为空)",
                    "included": True, "entry_id": e["entry_id"],
                })
...
def build_node_context(project_id: str, node_id: str, limit: int | None = None) -> str:
    ...
    with tx() as conn:
        ...
        # WPS 式大纲(任务词 2026-09-09):节点正文进对话上下文;老书 body 全空=零变化
        if node.get("body"):
            self_bits.append("正文:" + node["body"][:3000]
                             + ("…(截断)" if len(node["body"]) > 3000 else ""))
        if node["kind"] == "scene":
            try:
...
def build_book_context(project_id: str, limit: int | None = None) -> str:
    ...
    with tx() as conn:
        ...
        # 待议构思(批次七⑤:书级对话只喂待议,放弃/已采纳不进上下文;≤10 条)
        open_ideas = conn.execute(
            "SELECT title, note FROM idea_nodes WHERE project_id=? AND status='open'"
            " ORDER BY created_at DESC LIMIT 10", (project_id,)).fetchall()
        if open_ideas:
            ilines = [f"- {r['title']}" + (f":{r['note']}" if r["note"] else "")
                      for r in open_ideas]
            sections.append({"kind": "always", "title": "待议构思(开放讨论中)",
                             "content": "\n".join(ilines)})
