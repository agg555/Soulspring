...
def check_banned_patterns(text: str) -> List[str]:
    return check_banned_patterns_orig(text)


# ── 结构类规则库(rules-as-data,批次三算法体检 2026-09-06)──────────────
# 规则文件 anti_ai_rules.json 可热更:每次分析按 mtime 重载,零 LLM 调用。
# 来源:去AI味检索 A/B 档(humanizer/avoid-ai-writing/anti-ai-tell,全 MIT),
# 英文模式已中文网文本地化改写。详见 docs\参考\参考-去AI味技能检索-2026-09-06.md。
_RULES_PATH = Path(__file__).resolve().parent / "anti_ai_rules.json"
_rules_cache: tuple[float, dict] | None = None


def load_structural_rules() -> dict:
    """读结构规则(按 mtime 热更);文件缺失/损坏时返回空规则集,不阻塞审计。"""
    global _rules_cache
    try:
    ...
    try:
        ...
        return {}
    _rules_cache = (mtime, rules)
    return rules


def check_structural_rules(text: str) -> List[str]:
    """结构类检查:连续排比/对称段落/高频AI连接词/不是而是密度/章尾升华腔。

    与词汇类(fatigue)互补——anti-ai-tell 的 linter 范式:规则可正则化的不劳 LLM。
    """
    ...
    # 高频 AI 连接词/腔调词(密度)
    conn = rules.get("connectives", {})
    hits = sum(text.count(w) for w in conn.get("words", []))
    if hits / kilo > conn.get("ratio_per_kilo", 2.5):
        findings.append(f"[high]高频AI腔调词:{hits} 处(>{conn.get('ratio_per_kilo')}/千字)"
                        f";示例:{', '.join(w for w in conn.get('words', []) if w in text)[:60]}")

    # 连续排比:相邻句句首 4 字相同 或 句长接近的连续句
    par = rules.get("parallelism", {})
    run = 1
    for i in range(1, len(sentences)):
        a, b = sentences[i - 1], sentences[i]
        same_start = a[:4] == b[:4] and len(a) > 3
        close_len = abs(len(a) - len(b)) <= max(len(a), len(b)) * par.get("len_ratio", 0.3)
        if same_start or close_len:
            run += 1
            if run >= par.get("min_run", 3):
                findings.append(f"[{par.get('severity', 'high')}]连续排比(第{i - run + 2}~{i + 1}句:"
                                f"句首/句长高度相似)「{b[:24]}…」")
                run = 1
        else:
            run = 1

    # 对称段落:相邻两段首句等长(±2)且均无对话引号
    sym = rules.get("symmetry", {})
    paras = [p.strip() for p in text.split("\n") if p.strip()]
    for i in range(1, len(paras)):
        a, b = paras[i - 1], paras[i]
        if "「" not in a and "「" not in b and "“" not in a and "“" not in b \
                and abs(len(a) - len(b)) <= sym.get("len_tol", 2):
            findings.append(f"[{sym.get('severity', 'low')}]对称段落(第{i}/{i + 1}段长度几乎一致,总结腔指纹)")
            break

    # 不是…而是… 密度
    nxy = rules.get("not_x_but_y", {})
    cnt = len(re.findall(r"不是.{1,10}而是", text))
    if cnt / kilo > nxy.get("ratio_per_kilo", 1.2):
        findings.append(f"[{nxy.get('severity', 'low')}]「不是…而是…」{cnt} 次(密度超标,humanizer #9 本地化)")

    # 章尾升华腔:末 15% 内出现抽象总结句式
    gs = rules.get("grand_summary", {})
    tail = text[-max(1, int(n_chars * 0.15)):]
    for pat in gs.get("patterns", []):
        if pat in tail:
            findings.append(f"[{gs.get('severity', 'low')}]章尾升华腔:「{pat}…」出现在结尾(总结腔指纹)")
            break
    return findings


def check_banned_patterns_orig(text: str) -> List[str]:
    """检查禁用句式,返回命中的模式描述。"""
    issues = []
...
def analyze_text(text: str, extra_words: List[str] | None = None) -> Dict:
    ...
    structural = check_structural_rules(text)
    ...
    for f in structural:   # 结构类:high 扣 8 / low 扣 3(批次三算法体检)
        score -= 8 if f.startswith("[high]") else 3
