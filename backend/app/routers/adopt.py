...
- graph_add/subtopic_add:批准闸门,人确认(可增删改)后才落库
  (图谱节点落统一图谱引擎;子题树落 outline_nodes 的 topic 轻节点)。
"""
...
from .outline import PARENT_KINDS
...
# 子题树防御上限(任务词阶段 3):嵌套是特性,但模型/前端异常输入不能炸库
TOPIC_TREE_MAX_NODES = 100
TOPIC_TREE_MAX_DEPTH = 12
TOPIC_TITLE_MAX = 120


def sanitize_subtopic_tree(raw, _depth: int = 0) -> list[dict]:
    """子题树清洗:仅留 {title, children};空标题项丢弃;限深限长。

    split(AI 产出)与 adopt(人改后)共用一道清洗,落库形状恒一致。
    """
    if _depth > TOPIC_TREE_MAX_DEPTH:
        raise HTTPException(422, f"子题嵌套过深(上限 {TOPIC_TREE_MAX_DEPTH} 层)")
    if not isinstance(raw, list):
        raise HTTPException(422, "子题树应为数组")
    out: list[dict] = []
    for item in raw:
        ...
        if not title:
            continue
        out.append({"title": title[:TOPIC_TITLE_MAX],
                    "children": sanitize_subtopic_tree(item.get("children") or [], _depth + 1)})
    return out


def _count_tree(items: list[dict]) -> int:
    return sum(1 + _count_tree(i["children"]) for i in items)


def adopt_subtopic_tree(parent_nid: str, tree) -> dict:
    """子题树落地(闸门批准后的唯一落库路径;AI 永不直写 outline_nodes)。

    人已在前端闸门窗增删改并确认;此处再清洗一道(前后端契约以本函数为准),
    一次事务整树落 topic 轻节点(status 常量 'unwritten',无状态机不落时间戳),
    并留 agent_run 痕(agent_type=human_gate)。
    """
    clean = sanitize_subtopic_tree(tree)
    total = _count_tree(clean)
    if total == 0:
        raise HTTPException(422, "没有有效的子题(标题全为空)")
    if total > TOPIC_TREE_MAX_NODES:
        raise HTTPException(422, f"子题树过大(上限 {TOPIC_TREE_MAX_NODES} 个节点,当前 {total})")
    with tx() as conn:
        parent = conn.execute(
            "SELECT * FROM outline_nodes WHERE id=?", (parent_nid,)).fetchone()
        if parent is None:
            raise HTTPException(404, "目标节点不存在")
        if parent["kind"] not in PARENT_KINDS["topic"]:
            raise HTTPException(
                422, f"子题只能挂在 {'/'.join(sorted(PARENT_KINDS['topic']))} 下,"
                     f"不能挂在 {parent['kind']} 下")
        pid = parent["project_id"]
        now = _now()
        created = 0

        def _insert(items: list[dict], parent_id: str) -> None:
            nonlocal created
            for it in items:
                order = conn.execute(
                    "SELECT COALESCE(MAX(sort_order)+1, 0) FROM outline_nodes"
                    " WHERE project_id=? AND COALESCE(parent_id,'')=COALESCE(?,'')",
                    (pid, parent_id)).fetchone()[0]
                nid = f"node_{uuid.uuid4().hex[:20]}"
                conn.execute(
                    "INSERT INTO outline_nodes(id, project_id, parent_id, kind, title,"
                    " sort_order, status, status_changed_at, created_at, updated_at)"
                    " VALUES(?,?,?,'topic',?,?,'unwritten',NULL,?,?)",
                    (nid, pid, parent_id, it["title"], order, now, now))
                created += 1
                if it["children"]:
                    _insert(it["children"], nid)

        _insert(clean, parent_nid)
        conn.execute(
            "INSERT INTO agent_runs(id, project_id, node_id, action, agent_type, status,"
            " input_summary, finished_at, created_at) VALUES(?,?,?,?,?,?,?,?,?)",
            (f"run_{uuid.uuid4().hex[:20]}", pid, parent_nid,
             "adopt_subtopic_add", "human_gate", "succeeded",
             f"节点「{parent['title']}」下落子题树 {created} 个(人批准闸门)", now, now))
    return {"target": "subtopic_add", "node_id": parent_nid, "created": created,
            "summary": f"已在该节点下创建 {created} 个子题"}
...
class AdoptIn(BaseModel):
    ...
    tree: list | None = None    # subtopic_add 专用:人在闸门窗增删改后的树(覆盖 meta 原树)
...
def adopt_suggestion(body: AdoptIn) -> dict:
    ...
    elif target_type == "subtopic_add":
        result = _adopt_subtopic_add(target, override_tree=body.tree)
    else:
        raise HTTPException(422, "该建议没有可落地的目标(大纲字段/正文/事件/图谱对象/子题树可采纳)")
...
def _adopt_subtopic_add(target: dict, override_tree: list | None = None) -> dict:
    """子题树采纳(批准闸门,镜像 graph_add):前端闸门窗人已增删改确认,此处才落库。

    override_tree=闸门窗里人改过的树(优先);不传则落 meta 原树(等价只预览不改动)。
    """
    nid = target.get("node_id")
    tree = override_tree if isinstance(override_tree, list) else target.get("tree")
    if not nid:
        raise HTTPException(422, "subtopic_add 建议缺少 node_id")
    return adopt_subtopic_tree(str(nid), tree if isinstance(tree, list) else [])
