...
from ..settings_store import get_settings, ref_prompt_section, touch_prompt_append
import threading
...
def build_propose(pid: str) -> dict:
    ...
    system += touch_prompt_append("build_proposal")   # 批次六甲:触点追加(SCHEMA 之后,JSON 契约语义随附)
    system += ref_prompt_section("build_proposal", pid, None)
    ...
    try:
        result = chat_completion(
            ...
            # 上限=TOUCHES 登记 32000(批次六甲收编;GLM 思考量大,16k 会截断——实测口径随登记走)
        )
...
# ── 批次二 AI 流升级:构建任务化 + 字段级 AI 补写 ──

def _run_build_turn(tid: str, pid: str) -> None:
    """后台线程:整套构建提案(与同步版同逻辑),阶段可查、切页签不丢。"""
    key = pid   # create_task 注册键=pid(ACTIVE_NODES[pid])
    try:
        ...
        system = template.replace("{{BOOK_INFO}}", _book_info_block(book)).replace(
            "{{SCHEMA}}", _schema_block())
        system += touch_prompt_append("build_proposal")   # 批次六甲(后台管道同款)
        system += ref_prompt_section("build_proposal", pid, None)
        messages = [{"role": "system", "content": system},
                    {"role": "user", "content": "请生成整套设定草案,严格按系统指令输出 JSON。"}]
        result = chat_completion(
            messages, action="build_proposal", agent_type="builder",
            project_id=pid, input_summary=f"一键构建提案(后台): {book.get('name')}")
            # 上限=TOUCHES 登记 32000
        ...
        for item in raw_items:
            ...
            proposals.append(EntryIn(
                category=cat, name=str(item.get("name", "")).strip(),
                fields={k: str(v) for k, v in fields_in.items() if k in FIELD_LABELS[cat]},
                notes=str(item.get("notes", ""))))
        if not proposals:
            finish_task(tid, key, error="模型未产出任何有效条目(类别须为六类之一);可重试")
            return
        created = create_ai_proposals(pid, proposals)
        finish_task(tid, key,
                    result={"count": created["count"], "parsed_raw": len(raw_items),
                            "dropped": len(raw_items) - len(proposals),
                            "cost": result["usage"]["cost_total"]},
                    usage_total=result["usage"]["cost_total"])
    except Exception as exc:
        finish_task(tid, key, error=str(exc)[:2000])


@router.post("/{pid}/build/propose-async")
def build_propose_async(pid: str) -> dict:
    """AI 一键构建(后台任务):阶段进度可查,切页签/关面板不丢,完成后提案区可见。"""
    _load_book(pid)
    if ACTIVE_NODES.get(pid):
        raise HTTPException(409, "本书已有构建任务在跑,等它完成或刷新页面查看")
    row = create_task(pid, {"id": pid}, "l1_build", None)
    threading.Thread(target=_run_build_turn, args=(row["id"], pid), daemon=True).start()
    return {"task": row}
...
@router.post("/{pid}/build/suggest-field")
def suggest_field(pid: str, body: FieldSuggestIn) -> dict:
    """字段级 AI 补写(手工建档表单的"AI 补"按钮):按本书信息+类 schema 给单字段内容。"""
    book = _load_book(pid)
    if body.category not in FIELD_LABELS or body.field not in FIELD_LABELS[body.category]:
        raise HTTPException(422, "未知类别/字段")
    field_label = FIELD_LABELS[body.category][body.field]
    known = ";".join(f"{FIELD_LABELS[body.category].get(k, k)}={v}"
                     for k, v in body.known.items() if v)[:800]
    system = (f"你是网文设定助手。书名《{book.get('name')}》"
              f"({book.get('genre') or '类型未填'})。"
              f"为档案条目[{body.category}/{body.name or '(未命名)'}]填写字段"
              f"「{field_label}」。只输出该字段的填写正文,不要解释、不要前后缀,≤200 字。")
    system += touch_prompt_append("l1_field_fill")   # 批次六甲:触点追加(≤200 字约束之后,触点可放宽/收紧)
    result = chat_completion(
        [{"role": "system", "content": system},
         {"role": "user",
          "content": "已知信息:" + (known or "(无,自由发挥但须贴合书名类型)")}],
        action="l1_field_fill", agent_type="builder", project_id=pid,
        input_summary=f"字段补写:{body.category}.{body.field}")
    return {"text": result["content"].strip(), "cost": result["usage"]["cost_total"]}
