...
def _generate_draft_text(pid: str, node: dict, *, force_new_plan: bool,
    ...
    ref_prompt_ids: 参考提示词运行时手选(None=走绑定链:书>触点>全局)。
    """
    ...
    if not plan or force_new_plan:
        ...
        prompt += touch_prompt_append("chapter_plan")
        r = chat_completion(
            ...
            input_summary=f"计划卡:{node['title']}")  # 上限=TOUCHES 登记 8000(批次六甲)
        calls.append(r)
        try:
    ...
    prompt += touch_prompt_append("chapter_draft")
    prompt += ref_prompt_section("chapter_draft", pid, ref_prompt_ids)
    r = chat_completion(
        ...
        input_summary=f"草稿:{node['title']}")  # 上限=TOUCHES 登记 24000
    calls.append(r)
...
def _normalize(draft: str, calls: list, pid: str, node: dict) -> str:
    ...
    prompt += touch_prompt_append("chapter_normalize")
    r = chat_completion(
        ...
        input_summary=f"规整:{node['title']} ({count}字)")  # 上限=TOUCHES 登记 16000
    calls.append(r)
    out = r["content"].strip()
...
def _llm_review(pid: str, node: dict, draft: str, plan: dict) -> tuple[dict | None, float]:
    ...
    try:
        ...
        prompt += touch_prompt_append("chapter_review")
        r = chat_completion(
            [{"role": "system", "content": prompt},
            ...
            input_summary=f"评审:{node['title']}")  # 上限=TOUCHES 登记 8000
        try:
...
def _repair_once(pid: str, node: dict, skill: str | None,
                 ref_prompt_ids: list[str] | None = None) -> tuple[dict, float]:
    ...
    if skill_text:
        prompt += "\n\n" + skill_text
    prompt += touch_prompt_append("chapter_repair")   # 批次六甲:触点追加(技能之后,仍在协议语义外)
    prompt += ref_prompt_section("chapter_repair", pid, ref_prompt_ids)
    r = chat_completion(
        ...
        # 同草稿:max 档思考 + 完整正文
        input_summary=f"自修:{node['title']}")  # 上限=TOUCHES 登记 24000
    ...
    return ({"changeset": _changeset_view(fresh), "note": None,
             "usage_total": usage, "skill": skill,
             "request_tokens": int(r["usage"].get("request_tokens") or 0),
             "cached_tokens": int(r["usage"].get("cached_tokens") or 0)}, usage)
