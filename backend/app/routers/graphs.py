...
BOARD_KINDS = ("character", "event", "item", "map", "faction", "hook", "power", "free", "worldview", "system")
...
def board_detail(bid: str) -> dict:
    ...
    for e in edges:
        e["style"] = json.loads(e.get("style") or "{}")
...
class NodePatch(BaseModel):
    ...
    label: str | None = None
    sub_label: str | None = None
    style: dict | None = None   # 板语义挂载点(伏笔状态 status 等;整包覆盖)
...
def patch_node(nid: str, body: NodePatch) -> dict:
    ...
    if patch.get("style") is not None:
        patch["style"] = json.dumps(patch["style"], ensure_ascii=False)
    sets = ", ".join(f"{k}=?" for k in patch)
    with tx() as conn:
...
class EdgePatch(BaseModel):
    label: str | None = None
    kind: str | None = None
    style: dict | None = None   # 边语义挂载点(人物关系时间轴 at_chapter 等;整包覆盖)
