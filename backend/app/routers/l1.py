...
class EntryIn(BaseModel):
    ...
    fields: dict = {}
    notes: str = ""
    parent_entry_id: str | None = None   # 批次甲:父条目(同类别树形嵌套)
...
def create_entry(pid: str, body: EntryIn) -> dict:
    ...
    with tx() as conn:
        ...
        if body.parent_entry_id:
            parent = conn.execute(
                "SELECT * FROM l1_entries WHERE id=? AND project_id=?",
                (body.parent_entry_id, pid)).fetchone()
            if parent is None:
                raise HTTPException(404, "父条目不存在")
            if parent["category"] != body.category:
                raise HTTPException(422, f"父条目类别({parent['category']})与新建条目"
                                         f"({body.category})不一致;设定树只允许同类嵌套")
            if parent["entry_status"] != "confirmed":
                raise HTTPException(422, "父条目还是提案,批准后再挂子条目")
        conn.execute(
            "INSERT INTO l1_entries(id, project_id, category, name, fields, content,"
            " entry_status, source, parent_entry_id, created_at, updated_at)"
            " VALUES(?,?,?,?,?,?,?,?,?,?,?)",
            (eid, pid, body.category, body.name.strip(),
             json.dumps(_check_fields(body.category, body.fields), ensure_ascii=False),
             body.notes, "confirmed", "manual", body.parent_entry_id, now, now),
        )
...
def update_entry(eid: str, patch: dict) -> dict:
    with tx() as conn:
        ...
        # 批次甲:换父/提降级(同类、非自身、不得挂到自己的后代上成环)
        new_parent = patch.get("parent_entry_id", "__keep__")
        parent_sets = ""
        if new_parent != "__keep__":
            if new_parent is None:
                parent_sets, parent_args = "parent_entry_id=NULL", []
            else:
                if new_parent == eid:
                    raise HTTPException(422, "不能把自己挂成自己的子条目")
                p = conn.execute(
                    "SELECT * FROM l1_entries WHERE id=? AND project_id=?",
                    (new_parent, entry["project_id"])).fetchone()
                if p is None or p["category"] != entry["category"]:
                    raise HTTPException(422, "父条目不存在或类别不一致(设定树只允许同类嵌套)")
                walk = p
                while walk is not None and walk["parent_entry_id"]:
                    if walk["parent_entry_id"] == eid:
                        raise HTTPException(422, "不能把条目挂到自己的后代下面(会成环)")
                    walk = conn.execute(
                        "SELECT * FROM l1_entries WHERE id=?", (walk["parent_entry_id"],)).fetchone()
                parent_sets, parent_args = "parent_entry_id=?", [new_parent]
        else:
            parent_args = []
        # 人手工编辑不改变状态与来源:提案被编辑后仍是提案,批准流程不变
        set_parts = ["name=?", "fields=?", "content=?"] + (parent_sets.split(", ") if parent_sets else [])
        set_parts.append("updated_at=?")
        conn.execute(
            f"UPDATE l1_entries SET {', '.join(set_parts)} WHERE id=?",
            (name, json.dumps(fields, ensure_ascii=False), notes,
             *parent_args, _now(), eid),
        )
...
def delete_entry(eid: str) -> dict:
    with tx() as conn:
        ...
        if row["category"] == STYLE_FINGERPRINT:
            raise HTTPException(403, "风格指纹由蒸馏管道独占管理")
        # 批次甲:级联收集整棵子树(设定树删大节点=连小设定/技能条目一起删)
        ids = [eid]
        frontier = [eid]
        while frontier:
            fmarks = ",".join("?" * len(frontier))
            kids = [r["id"] for r in conn.execute(
                f"SELECT id FROM l1_entries WHERE parent_entry_id IN ({fmarks})",
                frontier).fetchall()]
            ids.extend(kids)
            frontier = kids
        marks = ",".join("?" * len(ids))
        conn.execute(f"DELETE FROM l1_entries WHERE id IN ({marks})", ids)
        for did in ids:
            ...
            graph_nids = [r["id"] for r in conn.execute(
                "SELECT id FROM graph_nodes WHERE ref_type='l1_entry' AND ref_id=?",
                (did,)).fetchall()]
            if graph_nids:
                gmarks = ",".join("?" * len(graph_nids))
                conn.execute(
                    f"DELETE FROM graph_edges WHERE from_node_id IN ({gmarks})"
                    f" OR to_node_id IN ({gmarks})", (*graph_nids, *graph_nids))
                conn.execute(f"DELETE FROM graph_nodes WHERE id IN ({gmarks})", graph_nids)
