...
def entity_links(pid: str, etype: str = Query(...), id: str = Query(...)) -> dict:
    ...
    with tx() as conn:
        if etype == "outline_node":
            ...
            if entry["name"]:
                like = f"%{entry['name']}%"
                # 体感三桶 2026-09-05(第三轮):关联性太弱——补"提到它的节点/事件"
                add("提到它的节点", [{**dict(r), "etype": "outline_node"} for r in conn.execute(
                    "SELECT id, title, kind AS extra FROM outline_nodes"
                    " WHERE project_id=? AND (title LIKE ? OR summary LIKE ?"
                    " OR scene_fields LIKE ?) ORDER BY sort_order LIMIT 20",
                    (pid, like, like, like)).fetchall()])
                add("提到它的事件", [{**dict(r), "etype": "timeline_event"} for r in conn.execute(
                    "SELECT id, title, time_label AS extra FROM timeline_events"
                    " WHERE project_id=? AND (title LIKE ? OR summary LIKE ?)"
                    " ORDER BY sort_key LIMIT 20", (pid, like, like)).fetchall()])
