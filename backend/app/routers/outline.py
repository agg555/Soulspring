"""L3 大纲树 API(M2,精修期第二批 C1/C2 扩展;WPS 式大纲 2026-09-09 扩)。
...
WPS 式子题(topic):卷/近纲/章/topic 下均可挂,可无限嵌套;纯轻节点——无状态机、
无场景字段、不计章数;正文走 body 列(章正文仍走 l4)。
...
KIND_ORDER = {"category": 0, "volume": 1, "arc": 2, "chapter": 3, "scene": 4, "topic": 5}
# 每种节点允许的父类型集合(近纲可选:chapter ∈ {volume, arc};scene 挂 chapter;
# topic=WPS 式子题,卷/近纲/章/topic 均可挂 → 子题可无限嵌套)
PARENT_KINDS = {
    "volume": {"category"},
    "arc": {"volume"},
    "chapter": {"volume", "arc"},
    "scene": {"chapter"},
    "topic": {"volume", "arc", "chapter", "topic"},
}
# topic 轻节点红线(任务词):无状态机、无场景字段、不计章数;章仍只能挂卷/近纲
TOPIC_BODY_MAX = 20000   # body 长度上限(任务词阶段 1:校验长度上限)
...
TRANSITIONS = {
    "unwritten": {"draft"},
    "draft": {"human_editing"},
    # 二期②:重 roll 自动回落——工作台在该态出新草稿时状态有处可去(实锤:
    # human_editing 唯一出边曾是待终审,无回草稿路,标签与内容脱节)
    "human_editing": {"final_review", "draft"},
    "final_review": {"finalized", "human_editing"},  # 打回:待终审 → 人改中
    # 二期②:定稿解封(用户拍板"定稿之后还可以修改")→ 人改中;前端二次确认
    "finalized": {"human_editing"},
}
...
class NodePatch(BaseModel):
    # C2 节点抽屉:标题/摘要/备注/正文人可编辑;至少给一项
    title: str | None = None
    summary: str | None = None
    note: str | None = None
    body: str | None = None   # WPS 式节点正文(卷/总纲/子题;章正文仍走 l4)
...
def update_node(nid: str, body: NodePatch) -> dict:
    ...
    if body.body is not None:
        if len(body.body) > TOPIC_BODY_MAX:
            raise HTTPException(422, f"正文超长(上限 {TOPIC_BODY_MAX} 字,当前 {len(body.body)} 字)")
        sets.append("body=?")
        params.append(body.body)
    if not sets:
        raise HTTPException(422, "无字段可更新(title/summary/note/body 至少一项)")
