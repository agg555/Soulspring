...
from .task_runner import (
    StepRecorder, create_task, finish_task, heal_stale, set_stage, task_view)
...
def _replace_changeset(pid: str, node: dict, draft: str, plan: dict,
    ...
    with tx() as conn:
        ...
        if node["status"] in ("unwritten", "human_editing"):
            # 生成草稿 = 未写→草稿;二期②:人改中重 roll 自动回落草稿(回边已开)
...
def preview(nid: str, project_id: str) -> dict:
    ...
    return {
        ...
        "changeset": _changeset_view(cs_live) if cs_live else None,
...
def _run_generation(tid: str, pid: str, node: dict, skill: str | None, kind: str,
                    ref_prompt_ids: list[str] | None = None) -> None:
    """后台线程:跑完整管道,阶段写 gen_tasks.stage;异常兜底落 error,不让线程裸死。

    批次七③:阶段推进同步记步骤明细(gen_task_steps),任务卡展开即时间线;
    产物摘要(计划卡键数/草稿字数/审计项数/评审维度)随步骤回填。
    """
    ...
    try:
        ...
        if kind == "draft":
            ...
            draft, plan, calls = _generate_draft_text(
                pid, node, force_new_plan=False, skill=skill,
                ref_prompt_ids=ref_prompt_ids, progress=progress)
            model = calls[0]["model"] if calls else ""
            rec.annotate("plan", tool=f"chat_completion@{model}",
                         artifact=(f"计划卡 {len(plan)} 键" if plan else "计划卡失败,按装配体直写"))
            rec.annotate("draft", tool=f"chat_completion@{model}",
                         artifact=f"草稿 {len(draft)} 字")
            progress("audit")
            validations, _ = _code_audit(pid, node, draft)
            rec.finish_current(tool="code_audit",
                               artifact=f"审计 {len(validations)} 项:" +
                                        ",".join(sorted({v["code"] for v in validations}))[:120])
            review = None
            review_err = None
            review_cost = 0.0
            if get_settings()["workbench"].get("llm_review", True):
                ...
                if isinstance(review, dict) and review.get("review_error"):
                    ...
                    rec.finish_current(status="error", artifact=str(review_err)[:120])
                else:
                    dims = (review.get("scores") if isinstance(review, dict) else None) or \
                           (review.get("dimensions") if isinstance(review, dict) else None)
                    rec.finish_current(artifact=(
                        f"评审 {len(dims)} 维" if isinstance(dims, dict) and dims
                        else f"评审回包 {len(str(review))} 字"))
            ...
            # 批次六乙:回执透传容量两值(数据已在 usage;与对话台占用条对齐)
            toks = {
            ...
            result = {"changeset": view, "note": review_err, "usage_total": usage, **toks}
            # 下限告警线(2026-09-09):管道内任一调用低于下限,随任务结果透传
            warns = [c.get("limit_warning") for c in calls if c.get("limit_warning")]
            if warns:
                result["limit_warning"] = ";".join(warns)[:300]
        else:
            progress("repair")
            result, usage = _repair_once(pid, node, skill, ref_prompt_ids)
            rec.finish_current(artifact=f"自修完成,本轮 ¥{usage:.4f}")
        rec.close()
...
def save_human_edit(nid: str, project_id: str, body: DraftIn) -> dict:
    ...
    _carry_dismissals(validations, _changeset_view(cs)["validations"])
    with tx() as conn:
        conn.execute(
