...
def test_v11_migration_from_character_relations(tmp_db):
    ...
    with tx() as conn:
        ...
        assert ver == 22   # v12 缓存列+…+v20 构思树(七⑤)+v21 l1 树形化(甲)+v22 节点 body
        cols = {r["name"] for r in conn.execute(
            "PRAGMA table_info(ai_usage_logs)").fetchall()}
        assert "cached_tokens" in cols
        ecols = {r["name"] for r in conn.execute(
            "PRAGMA table_info(graph_edges)").fetchall()}
        assert "style" in ecols
...
def test_price_for_discount_expiry(tmp_db):
    ...
    # 未配截止日 → 折扣
    assert price_for("glm-5.3-flash")[:2] == (0.4, 1.4)
    # 截止日 = 明天 → 仍折扣
    update_settings("pricing", {"discount_until": (date.today() + timedelta(days=1)).isoformat()})
    assert price_for("glm-5.3-flash")[:2] == (0.4, 1.4)
    # 截止日 = 昨天 → 自动正价
    update_settings("pricing", {"discount_until": (date.today() - timedelta(days=1)).isoformat()})
    assert price_for("glm-5.3-flash")[:2] == (0.8, 2.8)
    # 截止日 = 今天(含) → 折扣
...
def test_node_edge_style_patch(tmp_db):
    """批次二图谱板扩展:节点/边 style 可挂语义(伏笔状态/关系起于章)。"""
    from app.routers.graphs import (
        BoardIn, EdgeIn, EdgePatch, NodeIn, NodePatch,
        board_detail, create_board, create_edge, create_node, patch_edge, patch_node,
    )
    _seed_book_with_relation()
    b = create_board("p1", BoardIn(kind="hook", name="伏笔板"))["board"]
    na = create_node(b["id"], NodeIn(label="残纸"))["node"]
    nb = create_node(b["id"], NodeIn(label="鲸背区"))["node"]
    patch_node(na["id"], NodePatch(style={"status": "埋设"}))
    e = create_edge(b["id"], EdgeIn(from_node_id=na["id"], to_node_id=nb["id"],
                                    kind="承接"))["edge"]
    patch_edge(e["id"], EdgePatch(style={"at_chapter": 3}))
    out = {n["label"]: n["style"] for n in board_detail(b["id"])["nodes"]}
    assert out["残纸"] == {"status": "埋设"}
    e2 = {x["id"]: x["style"] for x in board_detail(b["id"])["edges"]}[e["id"]]
    assert e2 == {"at_chapter": 3}
