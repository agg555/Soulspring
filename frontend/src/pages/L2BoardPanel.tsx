...
export default function L2BoardPanel({ pid }: { pid: string }) {
  ...
  const redraft = async () => {
    ...
    try {
      ...
      // 批次二 AI 流升级:起草后台任务化,轮询阶段与成本
      const { task } = await api.draftL2Async(pid, redraftNid, text);
      setBusy("后台起草中(调用模型)…");
      const poll = window.setInterval(async () => {
        try {
          ...
          if (t.status === "done") {
            ...
            setBusy("");
            flash(`起草完成:${(t.result as unknown as { count?: number } | null)?.count ?? "?"} 类变更草案待批;成本 ¥${(t.usage_total ?? 0).toFixed(4)}`);
            load();
          } else if (t.status === "error") {
            window.clearInterval(poll);
            setBusy("");
            setError(t.error ?? "起草失败");
          }
        } catch { /* 轮询抖动忽略 */ }
      }, 2500);
      load();
  ...
  return (
    <div>
      ...
      <h3>伏笔池看板(当前第 {curChapter} 章，超 {staleThreshold} 章未回收标红)</h3>
