...
import { truncText } from "../graphLayout";
...
export default function TimelinePanel({ pid, onShowLinks }: {
  ...
  const [view, setView] = useState<"axis" | "cards" | "graph">("axis");
        ...
        <div className="tl-main">
          ...
          {view === "graph" ? (
            <div className="tl-graph">
              <svg width={shown.length * 176 + 130} height={236}>
                {["主线", "支线"].map((ln, li) => (
                  <g key={ln}>
                    <text x={12} y={80 + li * 96} fontSize={12} fill="var(--muted)">{ln}</text>
                    <line x1={52} y1={76 + li * 96} x2={shown.length * 176 + 100}
                      y2={76 + li * 96} stroke="var(--border)" strokeDasharray="4 4" />
                  </g>
                ))}
                {shown.map((e, i) => {
                  const lane = e.line === "支线" ? 1 : 0;
                  const x = 66 + i * 176, y = 46 + lane * 96;
                  return (
                    <g key={e.id} style={{ cursor: "pointer" }} onClick={() => setOpenEvt(e)}>
                      <title>{`${e.time_label || ""} ${e.title}(${e.status})点开编辑`}</title>
                      {i < shown.length - 1 && (
                        <line x1={x + 148} y1={y + 30} x2={x + 176} y2={y + 30}
                          stroke="var(--border)" />
                      )}
                      <rect x={x} y={y} width={148} height={60} rx={8}
                        fill="var(--panel)"
                        stroke={e.status === "已定" ? "var(--ok)" : "var(--border)"}
                        strokeWidth={1.6} />
                      <text x={x + 74} y={y + 20} textAnchor="middle" fontSize={11}
                        fill="var(--muted)">{e.time_label || "(未标时间)"}</text>
                      <text x={x + 74} y={y + 42} textAnchor="middle" fontSize={12}
                        fill="var(--text)">{truncText(e.title, 148)}</text>
                    </g>
                  );
                })}
              </svg>
