...
export interface L1Entry {
  ...
  source: string;
  parent_entry_id?: string | null;   // 批次甲:设定树父条目(同类别自由嵌套)
  created_at: string;
  updated_at: string;
}
...
export interface OutlineNode {
  ...
  // topic=WPS 式子题(卷/近纲/章/topic 下可挂,可无限嵌套;无状态机轻节点)
  kind: "category" | "volume" | "arc" | "chapter" | "scene" | "topic";
  title: string;
  sort_order: number;
  ...
  summary?: string | null;
  note?: string | null;
  body?: string | null;   // WPS 式节点正文(卷/总纲/子题;章正文仍走 l4)
...
export interface WorkbenchPreview {
  ...
  request_tokens?: number; cached_tokens?: number;
...
export interface LlmSettings {
  ...
  model: string;
  temperature: number;
  top_p?: number | null;   // 二期⑧温度预设(灵感=1.0+0.95;null=不传,厂商默认)
  max_tokens: number;
}
...
export interface AssemblySettings {
  ...
  body_inject?: boolean;   // WPS 大纲:节点正文装配注入(默认关)
}
...
export interface Settings {
  ...
  ui: { auto_compact: boolean; organizer_enabled: boolean };
  touches: Record<string, { prompt_append?: string; model?: string; max_tokens?: number }>;
  touches_registry: Record<string, { label: string; kind: string; hosts: string; max_tokens: number | null }>;
  prompt_templates?: { items: PromptTemplate[] };
...
export interface GenTask {
  ...
  node_title?: string;
  project_name?: string;
  kind: "draft" | "repair" | "chat" | "l1_build" | "l2_redraft";
  skill: string | null;
  session_id?: string | null;   // 对话任务关联的会话线
  stage: GenStage;
  status: "running" | "done" | "error";
  error: string | null;
  result: { changeset: ChangesetView | null; note: string | null; usage_total?: number; request_tokens?: number; cached_tokens?: number } | null;
  usage_total: number | null;
  created_at: string;
  updated_at: string;
  steps?: GenTaskStep[];   // 批次七③:步骤明细时间线(轮询返回体加厚)
}

// 批次七③:任务步骤明细(工具/环节、参数摘要、状态、起止、中间产物摘要)
export interface GenTaskStep {
  id: string;
  seq: number;
  name: string;
  tool: string;
...
// WPS 大纲:子题树节点(AI 拆分建议/闸门编辑共用形状)
export interface SubtopicItem {
  title: string;
  children?: SubtopicItem[];
}

// 选择填空式预设模板(2026-09-09):用户自建,全部 AI 对话宿主共用
...
// 输出上下限三层(2026-09-09):书×触点>书默认>触点 touches>全局默认>登记
...
export interface OutputLimits {
  default: LimitPair;
  books: Record<string, { default: LimitPair; actions: Record<string, LimitPair> }>;
}

// 参考提示词库+三层绑定(命中层不叠加:手选>书>触点>全局)
export interface RefPromptItem {
  id: string;
  name: string;
  text: string;
}
export interface ReferencePrompts {
  items: RefPromptItem[];
  global_bind: string[];
...
// Suggestion target_type 扩展(第三批 E;WPS 大纲批 +subtopic_add)
export interface Suggestion {
  quote: string;
  issue: string;
  suggestion: string;
  severity: string;
  target_type: string;   // none | chapter_text | outline_field | event_field | graph_field | graph_add | subtopic_add
...
export interface ChatMessageMeta {
  ...
  limit_warning?: string | null;   // 下限告警线(2026-09-09)
  parse_error?: boolean;
...
export interface ConversationSession {
  ...
  owner_type: "review" | "chat_test" | "outline_node" | "branch" | "idea";
  owner_id: string;
  name: string;
  created_at: string;
...
export interface IdeaNode {
  ...
  title: string;
  note: string;
  status: "open" | "adopted" | "dropped";
  depth: number;   // 根=1,上限 3
...
export interface AttachmentRef {
  type: "chapter" | "entry" | "hook" | "file";
  id?: string | null;
  label: string;
  text?: string;   // file 型:txt/md 全文(批次三①附件)
}
...
export interface WordStats {
  today: { human: number; ai: number; ai_ms?: number };
  hour: { human: number; ai: number };
  chapter: { human: number; ai: number };
  last24h: { hour: string; human: number; ai: number; ai_ms?: number }[];
  week: { day: string; human: number; ai: number; ai_ms?: number }[];
  since: { human: number; ai: number };
  total?: { human: number; ai: number };   // 全库累计净额(2026-09-10 总字数显示)
  now: string;
}
...
export interface GraphEdge {
  ...
  label: string;
  kind: string;
  style?: Record<string, unknown>;   // 边语义(人物关系时间轴 at_chapter 等)
}
...
export interface SearchGroup {
  group: "chapter" | "outline" | "entry" | "timeline";
  label: string;
  items: SearchItem[];
}
...
// 批次七⑥:模板双层(预设题材包 + 用户模板,分组显示)
export interface PresetPack {
  key: string;
  name: string;
  description: string;
  board_count: number;
  boards: { name: string; kind: string; node_count: number }[];
}
...
export interface ChapterCard {
  ...
  summary: string;
  note: string;
  events: { id: string; title: string; time_label: string }[];
