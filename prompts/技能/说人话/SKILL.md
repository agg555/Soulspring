---
name: 说人话
description: "审稿型技能：先标出中英文文本里的AI套路问题（说人话、自然一点、别像模板、先标问题），再按需清理，保留事实、术语、语域。注意：纯润色请用去AI味（通用）或story-deslop（网文）。"
---
...
# v2 增补:中文网文 AI 味禁词表与结构模式(2026-09-06)

> 来源标注(工作流 v1.1):融合 blader/humanizer(35 模式,MIT)+ avoid-ai-writing
> (审计/改写双层,MIT)+ anti-ai-tell(linter 范式,MIT),**英文模式已按中文网文
> 语境本地化改写,非翻译**;配套离线检查见 backend\app\audit\anti_ai_rules.json
> (规则可热更,零 LLM)。朱雀检测=本技能的效果裁判。

## 一、中文网文禁词/禁腔(点击即查,写作时避用)

**腔调词(高频即指纹)**:然而 / 与此同时 / 不仅仅 / 某种意义上 / 值得玩味 / 值得一提 /
仿佛在诉说 / 似乎在宣告 / 宿命般 / 命运的齿轮 / 空气仿佛凝固 / 时间仿佛静止 / 无一例外

**总结腔/升华腔(章尾重灾区)**:这就是… / 那是一种… / 见证着 / 意味着 / 诉说着 /
回响着 / 或许,这就是……

**对冲连发(一遍意思反复说)**:「不是 A,而是 B」连用;「与其说…不如说」连用;
同义否定三连(不…不…也不是…)

**假具体**:数字+模糊量词堆砌(三十七道伤痕、第九百次——没有信息量的精确)

## 二、结构模式(比词更重要)

1. **连续排比**(#10/#11):相邻 3+ 句句首相同或句长雷同——网文里最多见的是
   「他一步一步…他一步一步…」;拆:只保留一组,其余换成动作/对话;
2. **对称段落**(#27/#31):相邻两段长度几乎一致、结尾都在抒情=散文诗腔;拆:
   一段写实一段留白,长短错开;
3. **章尾升华**(全文末 15% 重灾区):结尾禁止抽象总结句,停在动作、对话或
   一个具体物件上;
4. **对称三连**(#10):三个短句/三个名词强凑一组——保留两个,第三个换成转折;
5. **段首重复开头**(#11):连续段落同词起手(「他知道…他知道…他知道…」)——
   一次是修辞,两次是腔。

## 三、使用纪律

- 本技能与 backend\app\audit\anti_ai_rules.json 同源同频:规则改 JSON 即生效,
  写作前可先自查结构五条(排比/对称/升华/对冲/重复开头);
- 改写优先级:结构 > 腔调词 > 禁词。只删词不改构=下一章指纹还在;
- 朱雀疑似率是事后裁判:每章跑完看分段,哪段红改哪段,别全文重写。
